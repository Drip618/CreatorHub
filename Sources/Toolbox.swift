            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    ToolboxSection(title: "🎬 创作增强 (Pro Workflow)", tools: [
                        ToolItem(icon: "doc.arrow.down", title: "XML 版本降级", subtitle: "兼容旧版 FCPX/PR", action: { downgradeXMLFlow() }),
                        ToolItem(icon: "trash.circle", title: "库缓存智能清理", subtitle: "清理渲染/代理文件", action: { cleanCacheFlow() }),
                        ToolItem(icon: "waveform.circle", title: "音频音量标准化", subtitle: "自动平衡 -14 LUFS", action: { normalizeAudioFlow() })
                    ])
